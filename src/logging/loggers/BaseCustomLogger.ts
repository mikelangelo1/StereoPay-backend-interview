...
import {
  NewRelicLoggerApiClient,
  NewRelicLoggerApiClientOptions,
} from '../loggerApiClients/NewRelicLoggerApiClient';
...
export type BaseCustomLoggerOptions = {
   ...
   */
  newRelicApiClientOptions?: NewRelicLoggerApiClientOptions;
...
export abstract class BaseCustomLogger {
  ...
  constructor(origin: string, options: Partial<BaseCustomLoggerOptions>) {
    this.origin = origin;
    if (options.newRelicApiClientOptions) {
      this.apiClient = new NewRelicLoggerApiClient(
        options.newRelicApiClientOptions,
      );
    }
